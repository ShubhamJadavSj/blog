...
<th {{ $attributes->merge(['scope' => 'col', 'class' => 'px-3 py-3.5 text-left text-sm font-semibold text-gray-900']) }}>
    <span class="group inline-flex"> {{ $slot }}
        <span class="ml-2 flex-none rounded bg-gray-200 text-gray-900 group-hover:bg-gray-300">
            @if(request('sort_type') == 'asc' && request('sort_by') == $sort)
                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                    <path fill-rule="evenodd" d="M14.707 12.707a1 1 0 01-1.414 0L10 9.414l-3.293 3.293a1 1 0 01-1.414-1.414l4-4a1 1 0 011.414 0l4 4a1 1 0 010 1.414z" clip-rule="evenodd"></path>
                </svg>
            @elseif(request('sort_type') == 'desc' && request('sort_by') == $sort)
