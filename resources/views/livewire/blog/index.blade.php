                            ...
                            <tr class="{{ $blog->trashed() ? 'opacity-50' : '' }}">
                                ...
                                <td class="relative py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                                    <div class="flex justify-end items-center space-x-4">
                                        ...
                                        @endcan
                                        @can('delete', $blog)
                                            <button wire:click="$emit('openConfirmModal', 'delete', {{ $blog->id }})" class="text-red-600 hover:text-red-800 cursor-pointer">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                        ...
                                        @endcan
                                        @can('restore', $blog)
                                            <button wire:click="$emit('openConfirmModal', 'restore', {{ $blog->id }})" class="text-indigo-600 hover:text-indigo-800 cursor-pointer">
                                        ...
                                        @can('forceDelete', $blog)
                                            <button wire:click="$emit('openConfirmModal', 'forceDelete', {{ $blog->id }})" class="text-red-600 hover:text-red-800 cursor-pointer"> {{-- archive => disabled --}}
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                        ...
                                        @endcan
                                        @can('publish', $blog)
                                            <button wire:click="$emit('openConfirmModal', 'publish', {{ $blog->id }})" class="inline-flex items-center justify-center rounded-md border border-transparent bg-indigo-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 sm:w-auto">
                                                Publish
                                            </button>
                                        @endcan
                                    </div>
